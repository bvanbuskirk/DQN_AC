...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for i in range(self.num_target_updates):
          next_vals = self(next_ob_no).squeeze()
          assert reward_n.size() == next_vals.size()
          y_n = (reward_n + self.gamma * next_vals * (1-terminal_n)).detach()

          for j in range(self.num_grad_steps_per_target_update):
            self.optimizer.zero_grad()
            vals = self(ob_no).squeeze()
            loss = self.loss(vals, y_n)
            loss.backward()
            self.optimizer.step()
